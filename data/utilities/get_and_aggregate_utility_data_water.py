...
if not os.path.isfile('propOAvgDict.json'):
    for prop in data:
        ...
        p = {'Record':'LocationName', 'SearchString':locn, 'ReportingGroup':'Water', 'StartTime':'2015-01-01', 'EndTime':'2019-12-12'}
        overallResponse = requests.get('https://helsinki-openapi.nuuka.cloud/api/v1.0/EnergyData/Monthly/ListByProperty', params=p)
        p2 = {'Record':'LocationName', 'SearchString':locn, 'ReportingGroup':'Water', 'StartTime':'2018-12-12', 'EndTime':'2019-12-12'}
...
with open('water_overall.csv', 'w+', newline='', encoding='utf8') as output:
    for elem in finalO:
        elem[1] = json.dumps(elem[1])
        output.write(";".join(map(str, elem))+"\n")

with open('water_lastyear.csv', 'w+', newline='', encoding='utf8') as output:
    for elem in finalY:
        elem[1] = json.dumps(elem[1])
        output.write(";".join(map(str, elem))+"\n")
